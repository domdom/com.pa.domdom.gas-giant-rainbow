...
void main()
{
    ...
    float baseColor = clamp(0.025 + stripes.x * 0.95 + (noise3 * 0.05), 0.0, 1.0) * 0.5 + 0.25 + time / 64 + v_Position_ObjectSpace.z / planetRadius;

    vec4 diffuse_raw = vec4(hsv2rgb(vec3(baseColor, 1, 1)), 1);
    vec4 fog_raw = vec4(hsv2rgb(vec3(baseColor, 0.8, 0.5)), 1);
